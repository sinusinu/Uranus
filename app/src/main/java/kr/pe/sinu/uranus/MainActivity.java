...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.ivMainMore.setOnClickListener(v -> {
            ...
            pwMoreWindow.showAtLocation(binding.ivMainMore, Gravity.CENTER, 0, 0);
        });
                                ...
                                if (arts != null && arts.length > 0) {
                                    ...
                                    try (var fos = new FileOutputStream(f)) {
                                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                                            artBitmap.compress(Bitmap.CompressFormat.WEBP_LOSSY, 85, fos);
                                        } else {
                                            //noinspection deprecation
    ...
    private void updateSpeedText() {
        if (bound) {
            int[] texts = {
                    ...
                    R.string.main_more_speed_0p90x,
                    R.string.main_more_speed_1x,
                    R.string.main_more_speed_1p10x,
                    R.string.main_more_speed_1p25x,
                    R.string.main_more_speed_1p50x,
                    R.string.main_more_speed_1p75x,
                    R.string.main_more_speed_2x,
            };
            mwBinding.tvMoreSpeedValue.setText(String.format(getString(R.string.main_more_speed_value), getString(texts[mps.getPlaybackSpeed() + 4])));
        }
    }
    ...
    private void loadSettings() {
        zen = sp.getInt("hide_album_art", 0) == 0;
        binding.ivMainCover.setVisibility(zen ? View.VISIBLE : View.INVISIBLE);
    }
